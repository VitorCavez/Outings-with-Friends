...
class _ProfilePageState extends State<ProfilePage> {
    ...
    try {
      ...
      if (msg.contains('PROFILE_PRIVATE')) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Profile is private')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      final resp = await _svc.fetchHistory(widget.userId, role: _historyRole, limit: 50, offset: 0);
      if (!mounted) return;
  ...
  Future<void> _loadTimeline() async {
    ...
    try {
      final resp = await _svc.fetchTimeline(widget.userId, limit: 50, offset: 0);
      if (!mounted) return;
  ...
  String _fmtRange(DateTime? start, DateTime? end) {
    ...
    if (start != null) return '${_dfDate.format(start)} • ${_dfTime.format(start)}';
                  ...
                  children: [
                    ...
                    Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      CircleAvatar(
                        ...
                        backgroundImage: p.profilePhotoUrl != null
                            ...
                            : null,
                        child: p.profilePhotoUrl == null ? const Icon(Icons.person, size: 36) : null,
                      ),
                      ...
                      Expanded(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          if (p.username != null)
                            Text('@${p.username}', style: const TextStyle(color: Colors.black54)),
                      ...
                      Column(children: [
                        const Text('Score'),
                        Text('${p.outingScore}', style: Theme.of(context).textTheme.titleLarge),
                      ]),
                    ]),
                    ...
                    if (p.badges.isNotEmpty) ...[
                      ...
                      Wrap(
                        spacing: 8,
                        runSpacing: 8,
                        children: p.badges.map((b) => BadgeChip(text: b)).toList(),
                      ),
                      const SizedBox(height: 16),
                    ],
                      ...
                      child: _loadingHistory
                          ...
                          : _history.isEmpty
                              ...
                              : ListView.separated(
                                  ...
                                  itemBuilder: (ctx, i) {
                                    final h = _history[i];
                                    final role = h.role ?? (h.createdById == widget.userId ? 'host' : 'guest');
                                    return ListTile(
                                      ...
                                      subtitle: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Text(_fmtRange(h.dateTimeStart, h.dateTimeEnd)),
                                          if (h.locationName != null)
                                            Text(h.locationName!, style: const TextStyle(color: Colors.black54)),
                                          if (h.rsvpStatus != null)
                                            Text('RSVP: ${h.rsvpStatus}', style: const TextStyle(color: Colors.black54)),
                                        ],
                                      ),
                          ...
                          : _favorites.isEmpty
                              ...
                              : ListView.separated(
                                  ...
                                  itemBuilder: (ctx, i) {
                                    ...
                                    final o = (item['outing'] ?? {}) as Map<String, dynamic>;
                                    ...
                                    final dt = o['dateTimeStart'] != null
                                        ? DateTime.tryParse(o['dateTimeStart'].toString())
                                        : null;
                                    final when = dt == null ? '' : _fmtRange(dt, null);
                                    ...
                                    return ListTile(
                                      ...
                                      subtitle: Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          if (loc != null) Text(loc, style: const TextStyle(color: Colors.black54)),
                                          if (when.isNotEmpty) Text(when, style: const TextStyle(color: Colors.black54)),
                                        ],
                                      ),
                                      trailing: const Icon(Icons.chevron_right),
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              Text(
                                                _fmtRange(e.dateTimeStart, e.dateTimeEnd),
                                                style: const TextStyle(color: Colors.black54),
                                              ),
                                              if (e.description != null && e.description!.isNotEmpty) ...[
                                                const SizedBox(height: 6),
                                                Text(e.description!, style: const TextStyle(height: 1.3)),
                                              ],
                                              if (linked != null) ...[
                                                const SizedBox(height: 6),
                                                InkWell(
                                                  // ✅ FIX: access properties instead of ['id']
                                                  ...
                                                  child: Row(
                                                    children: [
                                                      ...
                                                      Flexible(
                                                        child: Text(
                                                          // ✅ FIX: access properties instead of ['title']
