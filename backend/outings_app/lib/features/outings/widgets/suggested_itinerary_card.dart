...
class SuggestedItem {
  ...
  factory SuggestedItem.fromJson(Map<String, dynamic> j) => SuggestedItem(
    ...
    endTime: j['endTime'] != null ? DateTime.tryParse(j['endTime'] as String) : null,
...
class _SuggestedItineraryCardState extends State<SuggestedItineraryCard> {
  ...
  Future<void> _load() async {
    setState(() { _loading = true; _hidden = false; _items = []; });
    try {
      // 1) Check if current itinerary has items
      final r1 = await widget.api.get('/api/outings/${widget.outingId}/itinerary');
      if (r1.statusCode == 200) {
        final map = jsonDecode(r1.body) as Map<String, dynamic>;
        final List data = (map['data'] as List? ?? []);
        if (data.isNotEmpty) {
          if (mounted) setState(() { _hidden = true; _loading = false; });
          return; // do not show suggested if itinerary already exists
        }
      }

      // 2) Load suggested plan
      final r2 = await widget.api.get('/api/outings/${widget.outingId}/itinerary/suggested');
      ...
      final items = data2.map((e) => SuggestedItem.fromJson(e as Map<String, dynamic>)).toList().cast<SuggestedItem>();
  ...
  Future<void> _addAll() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Add all failed: $e')),
      );
    } finally {
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Expanded(
                  child: Text('Suggested plan', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                ),
                IconButton(
                  tooltip: 'Refresh',
              ...
              ..._items.take(5).map((e) => ListTile(
                ...
                subtitle: Text(
                  [
                    ...
                    if (e.startTime != null) 'Start: ${e.startTime}',
                    if (e.endTime != null) 'End: ${e.endTime}',
                  ].join(' • '),
                ),
              )),
              if (_items.length > 5)
                Padding(
                  ...
                  child: Text('+ ${_items.length - 5} more…', style: const TextStyle(color: Colors.black54)),
                ),
              Row(
                children: [
                  FilledButton.icon(
                    ...
                    label: _adding ? const Text('Adding…') : const Text('Add all to itinerary'),
                  ),
                ],
              )
            ],
