...
/* ───────── helpers ───────── */
function toIntOrNull(v){ if(v===undefined||v===null||v==='')return null; const n=Number(v); return Number.isFinite(n)?Math.trunc(n):null; }
function toFloatOrNull(v){ if(v===undefined||v===null||v==='')return null; const n=Number(v); return Number.isFinite(n)?n:null; }
function toDateIfString(v){ if(!v)return v; const d=typeof v==='string'?new Date(v):v; return d instanceof Date && !isNaN(d)?d:null; }
function centsFromFloat(v){ if(v===undefined||v===null)return null; const n=Number(v); if(!Number.isFinite(n))return null; return Math.round(n*100); }

const VisibilityEnum={ PUBLIC:'PUBLIC', CONTACTS:'CONTACTS', INVITED:'INVITED', GROUPS:'GROUPS' };
const ParticipantRoleEnum={ OWNER:'OWNER', PARTICIPANT:'PARTICIPANT', VIEWER:'VIEWER' };
// Match your Prisma enum: PENDING/ACCEPTED/DECLINED/CANCELED
const InviteStatusEnum={ PENDING:'PENDING', ACCEPTED:'ACCEPTED', DECLINED:'DECLINED', CANCELED:'CANCELED' };

function asVisibility(v){ if(!v)return null; const up=String(v).toUpperCase(); return VisibilityEnum[up]?up:null; }
function asParticipantRole(v){ if(!v)return ParticipantRoleEnum.PARTICIPANT; const up=String(v).toUpperCase(); return ParticipantRoleEnum[up]?up:ParticipantRoleEnum.PARTICIPANT; }
...
async function assertOwner(outingId,userId){
  const o=await prisma.outing.findUnique({ where:{ id:outingId }, select:{ id:true, createdById:true }});
  if(!o) return { ok:false, code:404, msg:'Outing not found' };
  if(o.createdById!==userId) return { ok:false, code:403, msg:'Only the organizer can perform this action' };
  return { ok:true };
}
...
async function isParticipant(outingId,userId){
  ...
  const row=await prisma.outingParticipant.findUnique({ where:{ outingId_userId:{ outingId, userId }}, select:{ userId:true }});
  return !!row;
}
async function hasInvite(outingId,userId){
  if(!userId) return false;
  const row=await prisma.outingInvite.findFirst({
    where:{ outingId, inviteeUserId:userId, status:{ in:[InviteStatusEnum.PENDING, InviteStatusEnum.ACCEPTED] } },
    select:{ id:true },
  });
  return !!row;
}
async function shareGroup(outingGroupId,userId){
  ...
  const row=await prisma.groupMembership.findFirst({ where:{ groupId:outingGroupId, userId }, select:{ id:true }});
  return !!row;
}
...
async function canViewOuting(outing, viewerId){
  ...
  switch(outing.visibility){
    case VisibilityEnum.PUBLIC: return true;
    case VisibilityEnum.CONTACTS: return viewerId ? await isContactEitherDirection(outing.createdById, viewerId) : false;
    case VisibilityEnum.INVITED: return viewerId ? await hasInvite(outing.id, viewerId) : false;
    case VisibilityEnum.GROUPS: return viewerId ? await shareGroup(outing.groupId, viewerId) : false;
...
async function canEditOuting(outing, viewerId){
  ...
  if(outing.createdById===viewerId) return true;
  if(outing.allowParticipantEdits && (await isParticipant(outing.id, viewerId))) return true;
  return false;
}
...
router.get('/', optionalAuth, async (req,res)=>{
  try{
    ...
    if(!userId){
      const items=await prisma.outing.findMany({ where:{ isPublished:true, visibility:'PUBLIC' }, orderBy:{ createdAt:'desc' }, take:50 });
      return res.json(items);
    }
    const [mine,pub]=await Promise.all([
      prisma.outing.findMany({ where:{ createdById:userId }, orderBy:{ createdAt:'desc' }, take:50 }),
      prisma.outing.findMany({ where:{ isPublished:true, visibility:'PUBLIC' }, orderBy:{ createdAt:'desc' }, take:50 }),
    ]);
    const map=new Map(); [...mine,...pub].forEach(o=>map.set(o.id,o));
...
router.post('/', requireAuth, async (req,res)=>{
  ...
  const {
    title, outingType, groupId, locationName,
    latitude, longitude, address,
    dateTimeStart, dateTimeEnd, description,
    budgetMin, budgetMax,
    piggyBankEnabled=false, piggyBankTarget, piggyBankTargetCents,
    checklist=[], suggestedItinerary, liveLocationEnabled=false,
    isPublic=false,
  } = req.body;
...
router.get('/:id/participants', requireAuth, async (req,res)=>{
  const { id } = req.params;
  try{
    const outing = await prisma.outing.findUnique({ where:{ id }, select:{ id:true, createdById:true, allowParticipantEdits:true }});
    ...
    const isPart=await prisma.outingParticipant.findUnique({
      where:{ outingId_userId:{ outingId:id, userId }}, select:{ id:true }
    });
    if(!owner && !isPart) return res.status(403).json({ error:'Not allowed to view participants' });
  ...
  try{
    ...
    const updated=await prisma.outing.update({
      ...
      data:{
        ...
        visibility:vis,
        allowParticipantEdits: typeof allowParticipantEdits==='boolean' ? allowParticipantEdits : undefined,
        showOrganizer: typeof showOrganizer==='boolean' ? showOrganizer : undefined,
      },
    });
...
router.post('/:id/invites', requireAuth, async (req, res) => {
  try {
    ...
    if (!outing.groupId) {
      const group = await prisma.$transaction(async (tx) => {
        ...
        await tx.groupMembership.upsert({
          where: { userId_groupId: { userId: inviterId, groupId: newGroup.id } },
          update: { role: GroupRole.admin, isAdmin: true },
          create: { userId: inviterId, groupId: newGroup.id, role: GroupRole.admin, isAdmin: true },
        });
      ...
      await prisma.groupMembership.upsert({
        where: { userId_groupId: { userId: inviterId, groupId: outing.groupId } },
        update: { role: GroupRole.admin, isAdmin: true },
        create: { userId: inviterId, groupId: outing.groupId, role: GroupRole.admin, isAdmin: true },
      });
    }

    // Collect invitees
    const { invitees, userIds = [], contacts = [], role, expiresInDays = 14 } = req.body || {};
    ...
    if (!normalized.length) return res.status(400).json({ error: 'No invite targets provided' });
    ...
    for (const i of normalized) {
      ...
      if (inviteeContact) {
        ...
        try {
          ...
          console.warn('Invite created but notify failed:', notifyErr?.message || notifyErr);
        }
      }
    }

    return res.status(201).json({ ok: true, invites: created, linkedGroupId: outing.groupId });
  } catch (e) {
...
router.get('/mine', requireAuth, async (req,res)=>{
  ...
  try{
    const [items,total]=await Promise.all([
      prisma.outing.findMany({ where:{ createdById:userId }, orderBy:{ createdAt:'desc' }, skip:offset, take:limit }),
...
router.get('/shared-with-me', requireAuth, async (req,res)=>{
  ...
  try{
    const rows=await prisma.outingParticipant.findMany({
      where:{ userId }, include:{ outing:true }, orderBy:{ createdAt:'desc' },
      skip:offset, take:limit
    });
    const filtered=rows.filter(r=>r.outing?.createdById!==userId);
    res.json({ ok:true, meta:{ total:filtered.length, limit, offset },
      data:filtered.map(r=>({ role:r.role, permissions:r.permissions, outing:r.outing })) });
  }catch(e){
...
router.get('/invites', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (me?.email) {
      orClauses.push({
        inviteeContact: { equals: me.email.toLowerCase().trim(), mode: 'insensitive' },
      });
    }
...
router.get('/sent-invites', requireAuth, async (req,res)=>{
  ...
  const status = statusParam && InviteStatusEnum[statusParam] ? statusParam : undefined;
...
router.patch('/invites/:inviteId', requireAuth, async (req, res) => {
  ...
  if (!['ACCEPT', 'DECLINE'].includes(actionRaw)) {
    return res.status(400).json({ error: 'INVALID_ACTION', details: 'action must be ACCEPT or DECLINE' });
  }
  ...
  try {
    ...
    const contactMatchesUser =
      ...
      (
        ...
        (me.phoneE164 && normPhone(me.phoneE164) === normPhone(contact))
      );
    ...
    if (inv.expiresAt && inv.expiresAt < new Date() && inv.status === InviteStatusEnum.PENDING) {
    ...
    if (actionRaw === 'DECLINE') {
      const declined = await prisma.outingInvite.update({
        where: { id: inviteId },
        data: { status: InviteStatusEnum.DECLINED, ...(inv.inviteeUserId ? {} : { inviteeUserId: userId }) },
      });
    ...
    const accepted = await prisma.$transaction(async (tx) => {
      ...
      await tx.outingParticipant.upsert({
        where: { outingId_userId: { outingId: updated.outingId, userId } },
        create: { outingId: updated.outingId, userId, role: updated.role || 'PARTICIPANT' },
        update: { role: updated.role || 'PARTICIPANT' },
      });

      // 👇 also add to the outing's group if there is one
      const o = await tx.outing.findUnique({ where: { id: updated.outingId }, select: { groupId: true } });
      if (o?.groupId) {
        await tx.groupMembership.upsert({
          where: { userId_groupId: { userId, groupId: o.groupId } },
          update: { role: GroupRole.member, isAdmin: false },
          create: { userId, groupId: o.groupId, role: GroupRole.member, isAdmin: false },
        });
      }
...
router.get('/:id/piggybank', optionalAuth, async (req,res)=>{
  ...
  try{
    ...
    if(!outing) return res.status(404).json({ error:'Outing not found' });
    const targetCents=outing.piggyBankTargetCents ?? centsFromFloat(outing.piggyBankTarget) ?? 0;
    const contributions=await prisma.outingContribution.findMany({ where:{ outingId:id }, orderBy:{ createdAt:'desc' }});
    const raisedCents=contributions.reduce((s,c)=>{
      if(typeof c.amountCents==='number' && Number.isFinite(c.amountCents)) return s+c.amountCents;
      if(c.amount!=null) return s+centsFromFloat(c.amount);
      return s;
    },0);
    const progressPct=targetCents>0 ? Math.min(100, Math.round((raisedCents/targetCents)*100)) : 0;
...
router.post('/:id/piggybank/contributions', requireAuth, async (req,res)=>{
  ...
  if(cents==null || cents<=0) return res.status(400).json({ error:'amountCents must be a positive integer.' });
  try{
    const outing=await prisma.outing.findUnique({ where:{ id }});
    if(!outing) return res.status(404).json({ error:'Outing not found' });
    if(!outing.piggyBankEnabled) return res.status(400).json({ error:'Piggy bank is not enabled for this outing.' });
    ...
    const contribution=await prisma.outingContribution.create({
      data:{ outingId:id, userId, amountCents:cents, amount:cents/100, note:note ?? null }
    });
...
router.post('/:id/expenses', requireAuth, async (req,res)=>{
  ...
  try{
    ...
    const expense=await prisma.expense.create({
      data:{ outingId:id, payerId, amountCents:cents, description:description ?? null, category:category ?? null }
    });
...
router.get('/:id/expenses/summary', optionalAuth, async (req,res)=>{
  const { id }=req.params;
  try{
    const expenses=await prisma.expense.findMany({ where:{ outingId:id }, orderBy:{ createdAt:'asc' }});
    const rsvps=await prisma.outingUser.findMany({ where:{ outingId:id }});
    const participantSet=new Set([ ...rsvps.map(r=>r.userId), ...expenses.map(e=>e.payerId) ].filter(Boolean));
    const participants=Array.from(participantSet);
    const totalCents=expenses.reduce((s,e)=>s+(e.amountCents||0),0);
    const count=participants.length||1;
    const perPersonCents=Math.round(totalCents/count);
    const paidMap={}; for(const pid of participants) paidMap[pid]=0;
    for(const e of expenses){ if(!paidMap[e.payerId]) paidMap[e.payerId]=0; paidMap[e.payerId]+=(e.amountCents||0); }
    const balances=participants.map(userId=>{
      const paidCents=paidMap[userId]||0;
      const balanceCents=paidCents - perPersonCents;
      return { userId, paidCents, owesCents:Math.max(0, perPersonCents - paidCents), balanceCents };
    });
    res.json({ totalCents, participants, perPersonCents, balances, expenses });
  }catch(e){
...
router.get('/:id', optionalAuth, async (req,res)=>{
  ...
  try{
    ...
    if(!allowed) return res.status(403).json({ error:'Not allowed to view this outing' });
...
function buildUpdateData(raw){
  ...
  if('dateTimeStart' in body) body.dateTimeStart = toDateIfString(body.dateTimeStart);
  if('dateTimeEnd' in body) body.dateTimeEnd = toDateIfString(body.dateTimeEnd);
  ...
  const normalizedTarget =
    ('piggyBankTargetCents' in body || 'piggyBankTarget' in body)
      ? (toIntOrNull(body.piggyBankTargetCents) ?? centsFromFloat(body.piggyBankTarget))
      : null;
  ...
  const allowed = [
    ...
    'budgetMin','budgetMax',
    'piggyBankEnabled','piggyBankTarget','piggyBankTargetCents',
    'allowParticipantEdits','showOrganizer','visibility','isPublished',
    'suggestedItinerary','checklist','liveLocationEnabled',
  ];
...
async function updateOutingHandler(req,res){
  ...
  try{
    ...
    if(!(await canEditOuting(existing, userId))){
      return res.status(403).json({ error:'You cannot edit this outing' });
    }

    // If enabling PB, ensure valid target
    const nextEnabled = ('piggyBankEnabled' in req.body) ? !!req.body.piggyBankEnabled : existing.piggyBankEnabled;
    const currentTargetCents = existing.piggyBankTargetCents ?? centsFromFloat(existing.piggyBankTarget) ?? null;
    const normalizedTarget =
      ('piggyBankTargetCents' in req.body || 'piggyBankTarget' in req.body)
        ? (toIntOrNull(req.body.piggyBankTargetCents) ?? centsFromFloat(req.body.piggyBankTarget))
        : null;
    const nextTargetCents = normalizedTarget != null ? normalizedTarget : currentTargetCents;
    if(nextEnabled === true && (!nextTargetCents || nextTargetCents <= 0)){
      return res.status(400).json({ error:'INVALID_PB_TARGET', details:'piggyBankTargetCents must be > 0 when piggyBankEnabled is true' });
    }
    ...
    if (e instanceof Prisma.PrismaClientValidationError) {
      return res.status(400).json({ error: 'VALIDATION_ERROR', details: e.message });
    }
    if (e.code === 'P2025') {
...
router.delete('/:id', requireAuth, async (req,res)=>{
  ...
  try{
    ...
    if(e.code==='P2025') return res.status(404).json({ error:'Outing not found' });
    res.status(500).json({ error:'Internal server error' });
  }
});
